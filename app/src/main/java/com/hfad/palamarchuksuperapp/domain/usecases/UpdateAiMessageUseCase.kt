...
interface UpdateAiMessageUseCase {
    suspend operator fun invoke(messageGroup: MessageGroup)
...
class UpdateAiMessageUseCaseImpl @Inject constructor(
    ...
    override suspend operator fun invoke(messageGroup: MessageGroup) {
        chatAiRepository.updateMessageGroup(messageGroup)
    }
